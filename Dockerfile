...
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name verso verso
...
# bind-mounting storage/ owns it as some other UID, and compose's `user:` is
# set to match — so Puma has to be able to create its pidfile and socket dir as
# an arbitrary UID, not just as 1000. Without this the container starts, binds
# its port, and then dies on
